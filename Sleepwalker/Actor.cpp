...
bool Actor::isOverlapDown(Actor* actor) {
	bool overlap = false;
	if (y + height > actor->y && // Parte inferior del actor actual
		y + height < actor->y + actor->height && // Parte superior del otro actor
		x + width > actor->x && // Lado derecho del actor actual
		x < actor->x + actor->width) { // Lado izquierdo del otro actor
		overlap = true;
	}
	return overlap;
}

bool Actor::isOverlapLeftRight(Actor* actor) {
	bool overlap = false;
	if (x + width / 2 < actor->x && // Lado derecho del actor actual
		x + width / 2 > actor->x - actor->width && // Lado izquierdo del otro actor
		(y < actor->y + actor->height && y + height / 2 > actor->y)) {
		overlap = true; // Colisión en la parte derecha del actor actual
	}
	else if (x > actor->x && // Lado izquierdo del actor actual
		x < actor->x + actor->width && // Lado derecho del otro actor
		(y < actor->y + actor->height && y + height / 2 > actor->y)) {
		overlap = true;
	}
	return overlap;
